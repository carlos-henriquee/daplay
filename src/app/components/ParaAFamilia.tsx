...
interface Filme {
 ...
 export default function ParaAFamilia() {
    const [show, setShow] = useState<Filme[]>([]);
      useEffect(()=>{
         fetch('http://localhost:8000/films')
         .then((res)=>res.json())
         .then((data)=>{
            const familyFilter = data.filter((show:Filme)=>show.classificacao_indicativa <=10 )
            setShow(familyFilter);
         })
      },[])
    ...
    return (
        <div className="absolute top-[70rem] m-10 flex flex-col">
            <h2 className="text-slate-600 text-2xl font-bold">Para a Família...</h2>
            <div className="flex flex-row mt-5">
               {show && show.map((show)=>(
                  <div key={show.id} className="w-52 mr-10 hover:scale-110 transition ease-in-out duration-200">
                     ...
                     <WatchButton  
                        onClick={()=>IncrementViews(show.id, show.views)}
                        endereço_do_filme={show.endereco_do_filme} 
                        categoria="normal"
